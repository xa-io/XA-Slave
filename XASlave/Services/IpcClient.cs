...
public sealed class IpcClient
{
    ...
    private readonly ICallGateSubscriber<string> xaGetFcNameSubscriber;
    private readonly ICallGateSubscriber<string> xaGetFcTagSubscriber;
    ...
    public IpcClient(IDalamudPluginInterface pluginInterface, IPluginLog log)
    {
        ...
        xaGetFcNameSubscriber = pluginInterface.GetIpcSubscriber<string>("XA.Database.GetFcName");
        xaGetFcTagSubscriber = pluginInterface.GetIpcSubscriber<string>("XA.Database.GetFcTag");
    ...
    {
        try { return xaGetFcNameSubscriber.InvokeFunc(); }
        catch { return string.Empty; }
    }

    public string GetFcTag()
    {
        try { return xaGetFcTagSubscriber.InvokeFunc(); }
        catch { return string.Empty; }
    }

    public int GetFcPoints()
    {
        try { return xaGetFcPointsSubscriber.InvokeFunc(); }
        catch { return 0; }
    }
    ...
    {
        try { return xaGetApartmentSubscriber.InvokeFunc(); }
        catch { return string.Empty; }
    }

    public string GetCharacterSummaryJson()
    {
        try { return xaGetCharacterSummaryJsonSubscriber.InvokeFunc(); }
        catch { return string.Empty; }
    }
