...
{
    private const string TalkBaseSig0 = "48 89 5C 24 ?? 48 89 6C 24 ?? 48 89 74 24 ?? 57 48 83 EC ?? 48 8B F9 48 8B EA 48 8B 49 ?? E8";
    private const string TalkBaseSig1 = "40 55 56 57 41 55 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 44 24";
    private const string TalkBaseSig2 = "48 89 54 24 ?? 53 56 57 41 54 41 55 48 83 EC ?? 48 8B D9";
    private const string TalkBaseSig3 =
        "48 89 54 24 ?? 53 57 41 54 41 55 41 56 48 83 EC ?? 48 8B D9 48 8B FA 48 8B 49 ?? E8 ?? ?? ?? ?? 4C 8B 25 ?? ?? ?? ?? 44 8B F0 48 8B 4B ?? 49 8B D4 4C 8B 2D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8B 4B ?? BA ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8B 4B ?? BA ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8B 4B ?? BA ?? ?? ?? ?? 85 C0 74 ?? E8 ?? ?? ?? ?? E9 ?? ?? ?? ?? 48 89 6C 24 ?? 4C 89 7C 24 ?? E8 ?? ?? ?? ?? 48 8B 4B ?? 49 8B D4 E8 ?? ?? ?? ?? 48 8B 4B ?? 45 33 C0 33 D2 E8 ?? ?? ?? ?? 41 83 C6 ?? 4C 8D 05 ?? ?? ?? ?? 41 8B D6 4D 8B CC 48 8B CB E8 ?? ?? ?? ?? 41 B9 ?? ?? ?? ?? 4C 8D 05 ?? ?? ?? ?? 41 8B D6 48 8B CB E8 ?? ?? ?? ?? 4C 8D 0D ?? ?? ?? ?? 41 8B D6 4C 8D 05 ?? ?? ?? ?? 48 8B CB E8 ?? ?? ?? ?? 4C 8D 0D ?? ?? ?? ?? 41 8B D6 4C 8D 05 ?? ?? ?? ?? 48 8B CB E8 ?? ?? ?? ?? 4C 8D 0D ?? ?? ?? ?? 41 8B D6 4C 8D 05 ?? ?? ?? ?? 48 8B CB E8 ?? ?? ?? ?? 4C 8D 0D ?? ?? ?? ?? 41 8B D6 4C 8D 05 ?? ?? ?? ?? 48 8B CB E8 ?? ?? ?? ?? 4C 8D 0D ?? ?? ?? ?? 41 8B D6 4C 8D 05 ?? ?? ?? ?? 48 8B CB E8 ?? ?? ?? ?? 4C 8D 0D ?? ?? ?? ?? 41 8B D6 4C 8D 05 ?? ?? ?? ?? 48 8B CB E8 ?? ?? ?? ?? E8";
    private const string TalkBaseSig4 =
    ...
    private void EnsureInitialized()
    {
        ...
        var talkBase0 = TryScanBaseAddress(TalkBaseSig0, "TalkBase0");
        ...
        var talkBase1 = TryScanBaseAddress(TalkBaseSig1, "TalkBase1");
        ...
        var talkBase2 = TryScanBaseAddress(TalkBaseSig2, "TalkBase2");
